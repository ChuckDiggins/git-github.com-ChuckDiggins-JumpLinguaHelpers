...
var myPhrasalVerbList: [JsonVerb] = [
    ...
    JsonVerb(spanish: "regístrese en", english: "check in",    french: "se présenter à l'enregistrement", transitivity: .ambitransitive, verbType : "NAC"),
]
...
//    }
    ...
    mutating public func encodeInternalVerbs(collectionType: JsonCollectionTypes, total: Int){
        ...
        case .Simple:
            wordList = myMultiLingualVerbListB
        }
        for v in wordList {
            myVerbList.append(v)
            if myVerbList.count >= total {break}
